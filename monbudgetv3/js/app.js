...
const fmtMoney = new Intl.NumberFormat("fr-CH", { style: "currency", currency: CURRENCY });
...
function renderPortfolio() {
  ...
  // Indicateurs globaux, convertis dans la devise de référence
  let invested = 0, value = 0, fees = 0, buyCostTotal = 0;
  for (const a of assets) {
    const s = assetStats(a), r = fxRate(a.currency);
    invested += s.invested * r;
    value += s.value * r;
    fees += s.fees * r;
    buyCostTotal += s.buyCostTotal * r;
  }
  const pv = value - invested;
  let pvNet = pv - fees;
  if (pvNet > 0) pvNet *= 1 - (inv().taxRate || 0) / 100;
  const pvPct = buyCostTotal > 0 ? pv / buyCostTotal * 100 : 0;

  setKpi("pf-invested", money(invested));
  setKpi("pf-value", money(value));
  setKpi("pf-pv", money(pv), pv);
  setKpi("pf-pvnet", money(pvNet), pvNet);
  ...
  const allTs = [...new Set(assets.flatMap(a =>
    [...pricePoints(a).map(p => p.ts), ...a.transactions.map(t => t.ts)]))].sort();
  ...
  chartPfAssets = new Chart(document.getElementById("chart-pf-assets"), {
    ...
    options: lineOptions(v => money(v), true)
  });
}
...
function renderPfSettings() {
  ...
  const currencies = [...new Set(inv().assets.map(a => a.currency))]
    .filter(c => c !== REF_CURRENCY);
...
document.getElementById("btn-update-fx").addEventListener("click", async () => {
  const currencies = [...new Set(inv().assets.map(a => a.currency))]
    .filter(c => c !== REF_CURRENCY);
  if (!currencies.length) { pfApiStatus("Tous vos actifs sont déjà en " + REF_CURRENCY + ".", true); return; }
  pfApiStatus("Récupération des taux…", true);
  const done = [], failed = [];
  for (const c of currencies) {
    try {
      const res = await fetch(`https://api.frankfurter.app/latest?from=${encodeURIComponent(c)}&to=${REF_CURRENCY}`);
      if (!res.ok) throw new Error(res.status);
      const data = await res.json();
      const rate = data && data.rates && data.rates[REF_CURRENCY];
      if (!(rate > 0)) throw new Error("taux absent");
      inv().fx[c] = rate;
      done.push(`1 ${c} = ${rate.toFixed(4)} ${REF_CURRENCY}`);
...
function renderAssetDetail(asset) {
  ...
  document.getElementById("as-pru").textContent = s.pru ? moneyIn(s.pru, asset.currency) : "—";
  ...
  setKpi("as-pv", moneyIn(s.pv, asset.currency), s.pv);

  // Graphique : cours + marqueurs achat/vente
  const pts = pricePoints(asset);
  ...
  document.getElementById("asset-tx-list").innerHTML = txs.map(t => `
    <div class="mini-row">
      ...
      <span class="grow">${tsLabel(t.ts)} — ${t.qty.toLocaleString("fr-CH", { maximumFractionDigits: 8 })} × ${moneyIn(t.price, asset.currency)}${t.fees ? ` <span class="muted">(+${moneyIn(t.fees, asset.currency)} frais)</span>` : ""}</span>
